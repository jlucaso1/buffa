...
| Type-erased decode loops (`merge_to_limit` / `merge_length_delimited` / `merge_group` run through `&mut dyn FieldMerge<B>`) | One indirect call per decoded field into the message's `merge_field`; measured −10% on `api_response/decode` (the loop no longer carries the whole inlined sub-tree) and +6% on `mesh/decode`: the vertex itself is tiny and keeps an inlined loop, the cost is the per-element call from the mesh's erased loop into its `merge_field` for every repeated vertex | One loop body per buffer type instead of one per (message, buffer) pair, and no sub-decoder inlined into every parent arm: −13% owned / −15% view on a 750-message schema at `opt-level = "z"` (Berkeley `size` text, which includes the one vtable per (message, buffer) pair the erasure adds). Messages with at most four singular fields keep monomorphic `#[inline]` loops (codegen overrides), so a vertex or timestamp still inlines into its parent's arm. |
