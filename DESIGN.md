...
| Type-erased decode loops (`merge_to_limit` / `merge_length_delimited` / `merge_group` run through `&mut dyn FieldMerge<B>`) | One indirect call per decoded field of a *nested* message into its `merge_field`; the top-level entry points (`merge`, `decode*`, `decode_view_ctx`) keep a monomorphic loop, because the erased loop's single call site is shared by every message type and mispredicts as parent and child alternate. Measured cost against `main`, with that split and the tiny-message overrides in place, in retired instructions (cachegrind, deterministic): `mesh/decode` (a flat list of 3-float vertices, the shape with the least work per call) 73.78 M → 73.78 M, `api_response/decode` 0.903 M → 0.903 M, i.e. none on either shape; `mesh/decode_view` 130.1 M → 121.1 M (−7%), because a repeated view element is now a default merged through the shared loop rather than a `decode_view_ctx` instantiation. Wall-clock on the `bench-iso` shapes moves ±10% between runs of the same binary on the measuring machine, so it is not used here. | One loop body per buffer type instead of one per (message, buffer) pair, and no sub-decoder inlined into every parent arm: −25% owned / −28% view on a 750-message schema, and −26% for an application that decodes and encodes only the 18 root messages it actually exchanges (all decoder and encoder changes in this release together) at `opt-level = "z"` with every message also decoded at top level (Berkeley `size` text, which includes the one vtable per (message, buffer) pair the erasure adds). Messages with at most four singular fields, none string or bytes, keep monomorphic `#[inline]` loops (codegen overrides), so a vertex or timestamp still inlines into its parent's arm. |
