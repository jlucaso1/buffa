...
| Type-erased decode loops (`merge_to_limit` / `merge_length_delimited` / `merge_group` run through `&mut dyn FieldMerge<B>`) | One indirect call per decoded field into the message's `merge_field` | One loop body per buffer type instead of one per (message, buffer) pair, and no sub-decoder inlined into every parent arm: −13% `.text` on a 750-message schema at `opt-level = "z"`. The per-field work (tag decode, wire-type check, value decode) dwarfs the call. |
