...
| Type-erased decode loops (`merge_to_limit` / `merge_length_delimited` / `merge_group` run through `&mut dyn FieldMerge<B>`) | One indirect call per decoded field into the message's `merge_field`; measured −10% on `api_response/decode` (the loop no longer carries the whole inlined sub-tree) and +6% on `mesh/decode`, a flat list of 3-float vertices where the per-element call is a visible fraction of the work | One loop body per buffer type instead of one per (message, buffer) pair, and no sub-decoder inlined into every parent arm: −13% owned / −15% view `.text` on a 750-message schema at `opt-level = "z"`. Messages with at most four singular fields keep monomorphic `#[inline]` loops (codegen overrides), so a vertex or timestamp still inlines into its parent's arm. |
